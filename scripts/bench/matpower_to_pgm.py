...
Why not go through pandapower's own MATPOWER importer instead (as
convert_pandapower_case.py does for other purposes)? Three of these twelve
cases (case1888rte, case6495rte, case6515rte) would not converge at all
through that path. Root-caused (not assumed) by comparing directly against
`references/powsybl-open-loadflow` via `pypowsybl`: pandapower's importer
(via power-grid-model-io's `PandaPowerConverter`) assigns each bus a real
physical `baseKV`, which for a transformer connecting two different voltage
levels requires very carefully keeping every impedance/tap conversion
referenced to a *consistent* side — easy to get subtly wrong (this script's
own first attempt did, twice). powsybl's own MATPOWER importer sidesteps
the whole problem: verified directly (`network.get_voltage_levels()
["nominal_v"]` after `pypowsybl.network.load(...)`) that it assigns
`nominal_v = 1.0` to *every* bus regardless of the `.m`/`.mat` file's
`baseKV` column, and encodes an off-nominal branch purely via
`rated_u1 = ratio`, `rated_u2 = 1.0`. MATPOWER's own power-flow formulation
never actually needs a physical voltage reference, only a *consistent*
one — so this script does the same (`U_RATED_UNIFORM` below), which
eliminates the whole class of from-side/to-side base-mismatch bug by
construction, and fixed convergence on all three previously-failing cases
immediately.

Requires `scipy` (`pip install scipy numpy`) for `.mat` reading — no
pandapower, no power-grid-model-io.
...
only a *consistent* one. An earlier version of this script used `baseKV`
...
nodes' `u_rated` (true of every hand-authored fixture in this project, and
of convert_pandapower_case.py's output) — which is exactly why an
off-nominal ratio has to be injected via a forced tap step here (`tap_min =
tap_max = tap_pos = 1`, `tap_nom = 0`, `tap_size = u1 * (ratio - 1)`)
instead of by scaling `u1` directly, which `transformer_tap` would ignore.
...
for the RTE cases' unusual near-zero-impedance phase-shifting branches (as
...
# and convert_pandapower_case.py.
SOURCE_SK = 1e10
SOURCE_RX_RATIO = 0.1
...
def parse_matpower_m(m_path: Path) -> dict:
    ...
    for name in ("bus", "gen", "branch"):
        block_match = __import__("re").search(rf"mpc\.{name}\s*=\s*\[(.*?)\];", text, __import__("re").DOTALL)
        if not block_match:
...
def convert(mat_path: Path, output_path: Path) -> None:
    ...
    # matching this project's convert_pandapower_case.py convention. Bus
    # type PV with at least one active gen: `sym_gen` (summed P across all
    # active gens at that bus) + one `voltage_regulator` referencing the
    ...
    output_path.write_text(__import__("json").dumps(output))


_next_id_counter = [10_000_000]
