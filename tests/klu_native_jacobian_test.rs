...
/// Mirrors `klu_jacobian_test.rs`'s own `solve_with_klu_backend`, but runs
/// `JacobianBackend::KluNative` (the pure-Rust port) instead of `Klu` (the
/// vendored-C-via-FFI backend) — no `klu` Cargo feature needed, so this
/// whole test file runs under plain `cargo test`.
fn solve_with_klu_native_backend(mut buses: Vec<Bus>, ybus: gridoxide::network::YBus) -> Vec<Bus> {
    let ybus = ybus.finish();
    linear_initial_guess(&mut buses, &ybus);
    newton_raphson_with_backend(&mut buses, &ybus, 1e-6, 20, JacobianBackend::KluNative);
    buses
}
...
fn assert_matches_pgm(base: &PathBuf, tol: f64) {
    ...
    let result = solve_with_klu_native_backend(buses, ybus);
...
#[test]
fn klu_native_backend_matches_pgm_basic_node() {
    assert_matches_pgm(&data_dir("basic-node"), 1e-5);
}

#[test]
fn klu_native_backend_matches_pgm_node() {
    assert_matches_pgm(&data_dir("node"), 1e-5);
}

#[test]
fn klu_native_backend_matches_pgm_line() {
    assert_matches_pgm(&data_dir("line"), 1e-5);
}

#[test]
fn klu_native_backend_matches_pgm_shunt() {
    assert_matches_pgm(&data_dir("shunt"), 1e-5);
}

#[test]
fn klu_native_backend_matches_pgm_sym_load() {
    assert_matches_pgm(&data_dir("sym_load"), 1e-5);
}

#[test]
fn klu_native_backend_matches_pgm_sym_gen() {
    assert_matches_pgm(&data_dir("sym_gen"), 1e-5);
}

#[test]
fn klu_native_backend_matches_pgm_source() {
    assert_matches_pgm(&data_dir("source"), 1e-5);
}

#[test]
fn klu_native_backend_matches_pgm_transmission_case() {
...
#[test]
fn klu_native_backend_matches_pgm_transformer_batch() {
...
#[test]
fn klu_native_backend_matches_pgm_distribution_case_batch() {
