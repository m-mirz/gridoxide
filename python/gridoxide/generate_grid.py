...
no tap changer / no measurements / no fault case. Generates synthetic PGM
JSON `input` documents at any scale — no external dependencies (pure
stdlib), so it's part of the core `gridoxide` package rather than gated
behind an extra.
...
JSON file in `scripts/bench/`, the comparison there is apples-to-apples
regardless of how the topology was generated.

CLI usage: `gridoxide-generate-grid <output.json> [--target-nodes N] [--seed N]`
(installed as a console script) or `python3 -m gridoxide.generate_grid
<output.json> [--target-nodes N] [--seed N]`.
...
were generated with:
    ...
    gridoxide-generate-grid grid_small.json  --target-nodes 200   # -> 192 nodes
    gridoxide-generate-grid grid_medium.json --target-nodes 1500  # -> 1,003 nodes
    gridoxide-generate-grid grid_large.json  --target-nodes 2200  # -> 2,605 nodes
