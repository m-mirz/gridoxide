...
9. **Remedial action optimization** — **Done**, and it is not a row any of the five comparison tools
   ...
   Cucumber suite, so 156 of its own scenarios are vendored verbatim and scored — re-run and
   unchanged as of this revision — 138/142 DC and
   ...
   Still absent: whatever is behind the 167 unmatched assertions, which is **not** what this note
   used to say it was. It claimed the reference blooms combinations at each depth where this search
   extends a single best chain; the reference's own `SearchTreeBloomer.bloom` is greedy too, over
   individual actions plus *configured* combinations, and no vendored configuration supplies one. The
   failures are curative-only and the cause is being measured rather than guessed at —
   `plans/RAO_SEARCH_PLAN.md`. Also absent:
