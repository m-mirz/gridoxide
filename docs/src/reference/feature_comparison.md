...
| Asymmetric / unbalanced power flow | ❌ symmetric only | ✅ | ✅ (`LfAsym*`) | ✅ (dedicated `Simulations/PowerFlow3ph/` driver) | ✅ (`runpp_3ph`) | ✅ phase domain throughout — three buses per node laid out `3·node + phase`, sequence-to-phase transforms for lines, transformers, shunts, links and three-winding transformers. Gated against power-grid-model's own asymmetric fixtures for lines, transformers and three-winding transformers (`tests/pgm_asym_*_test.rs`) |
...
| State estimation | ❌ | ✅ (WLS, **sym + asym**, iterative-linear + Newton-Raphson, voltage/power/**current** sensors, **batched** with topology caching and thread-parallelism; no bad-data detection) | ❌ | ✅ (WLS + observability analysis + pseudo-measurement augmentation) | ✅ (WLS, `estimation` module) | ✅ **both domains** (WLS + observability + bad-data detection + zero-injection constraints, both PGM calculation methods, **batched** with thread-parallelism and a shared factorization, voltage/power/**current** sensors in both angle frames). **Symmetric**, one bus per node. And **asymmetric**, in the phase domain: three buses per node laid out `3·node + phase`, with each asym sensor reading its own phase against a line-to-neutral voltage base rather than reducing to a balanced total — which is what makes an unbalanced network estimable rather than approximable, and on a distribution feeder the unbalance is the question. Nothing in the estimator changed for it: `Target`, `StateLayout`, the Jacobian, the constraints, both methods and the batch solver are the same code, because a three-phase branch terminal is a six-coefficient `CurrentFunctional` where a scalar one has two. `se::case::SeCase` is the single entry point for both, and `gridoxide estimate [--asymmetric]` reaches them. Gated against power-grid-model's published *asymmetric* answer for `transmission-case` (all 33 phase-buses to 1e-6), and — since the analyses were only ever claimed to carry over — observability, bad-data detection and the batch solver are each now gated in the phase domain too, the last bit-for-bit against a sequential loop. One limit found in the doing and gated rather than papered over: Gauss-Newton takes an undamped step, so a measurement far enough from consistent diverges — on this fixture at 20σ in the phase domain and 50σ symmetrically — and reports `MaxIterations` rather than a confident answer. What remains is a modelling difference about *sources*, described in the note below, and nothing else: every component the symmetric path reads, the phase domain now models |
...
| **Small-signal / modal** (eigenvalue) analysis | ❌ | ❌ | ❌ | ✅ `Simulations/SmallSignalStabilityRms/` + `SmallSignalStabilityEmt/` | ❌ | ⚠️ **the RMS half is complete; the EMT half is absent, and that is the only reason this is not ✅** (`src/dynamics/smallsignal.rs`): the DAE linearized about an equilibrium, the algebraic block eliminated (`A = A_x − A_v·C_v⁻¹·C_x`), eigenvalues, damping ratios, modal frequencies, **participation factors** naming the states each mode belongs to, and **mode shapes** saying how the rotors move relative to one another — which is what separates an inter-area mode from a local one. The four blocks are not re-derived — they are the ones `DaePattern::fill` already assembles for every Newton iteration, so the linearization cannot drift from the simulation it describes. **Two methods, and the result says which answered.** Dense (`analyze`) computes every mode and is the default below two thousand states. Past that, or whenever a shift is named, **sparse shift-invert Arnoldi** (`analyze_near`, `src/dynamics/arnoldi.rs`) computes the `k` modes nearest a point in the complex plane without ever forming `A`: the operator is the Schur complement of the *bordered* pencil, so `(A − σI)⁻¹b` is one sparse complex solve on the same `DaePattern` the simulation uses, and a shift adds no fill-in because the state diagonal is structurally present already (`src/dynamics/shift.rs`). Left eigenvectors — hence participation factors — come from a second Arnoldi pass on the adjoint operator against the same factorization; an ambiguous left/right match is reported *without* participation rather than with a plausible wrong one. **20 480 states (a 4 096-bus ring, 2 048 sixth-order machines with exciters and governors) in under 15 s, every residual under 3e-14** — where the dense method cannot start. Residuals are measured by applying the operator, not by the textbook estimate, which was measured reporting `8e-52` where the truth was `1.3e-14`. ✅ **Eigenvalue sensitivities** too: `dλ/dp`, `dζ/dp` and `df/dp` from a quadratic form in the pencil's own left and right eigenvectors, with `∂J/∂p` a central difference of the assembly rather than a second derivation — restricted, on purpose, to parameters the *equilibrium* does not depend on (`h`, `d`), since for a reactance the true derivative carries a `∂J/∂x·dx/dp` term this does not have. Gated against the closed-form swing eigenvalue `±j√(Ω_b·K_s/2H)` and its own derivative `dλ/dH = −λ/2H`; against a difference of two whole analyses; against the time-domain run's observed period (2e-3) and peak-to-peak decay (2%), which the *sparse* path also meets by exciting one mode along its eigenvector; and against **ARPACK** on the identical operator (opt-in `smallsignal-arpack`, own hand-written bindings to arpack-ng's `znaupd_c`/`zneupd_c`), which isolates the eigensolver from everything shared. CLI (`gridoxide dynamics --modes --modes-freq --modes-near --modes-sensitivity`) and Python (`small_signal`). Not here: the EMT half, and block Arnoldi for genuinely degenerate spectra — a ring of identical machines has near-exact repeated modes and a Krylov space finds one vector per invariant direction, which is gated as a limitation rather than left as a footnote — see [Small-Signal Analysis](../dynamics/smallsignal.md) |
| **Remedial action optimization** (CRAC / CNEC, preventive + curative) | ❌ | ❌ | ❌ — this is [powsybl-open-rao](https://github.com/powsybl/powsybl-open-rao)'s job, a separate project, and it is the reference gridoxide is gated against | not surveyed (its contingency row mentions SRAP support, which is adjacent) | not surveyed | ✅ `src/rao/`: CRAC model and readers (every one of the 428 CRACs in the reference checkout imports, across 24 format versions — a recorded sweep, see `plans/RAO_PLAN.md` phase 4, not a standing gate; what CI walks is the vendored subset under `tests/data/rao`), evaluation kernel with Woodbury outage screening, LP over range actions, search tree over network actions, CASTOR perimeter decomposition, automaton simulation, MNEC soft constraints, conditional usage rules, RA usage limits, curative stop criterion, AC re-validation. second preventive with its execution conditions. Gated against the reference's **own** Cucumber suite — 197 scenarios, **1825 of 1862** checkable assertions (180/186 DC, 276/282 + 963/977 AC, 118/123 second preventive, 288/294 costly optimization). Costly optimization (`MIN_COST`) in full: network actions, a range action's movement priced at its `variationCosts`, and its activation as a binary in a MIP. Of the 37 that differ, **32 are recorded disagreements** measured on the reference's own objective, in none of which gridoxide is worse — including two where it secures the network *more cheaply* than the reference asks; the other 5 are one open defect with seven mechanisms measured and refuted. Not yet: loop flows, relative margins, HVDC and counter-trade range actions, an angle/voltage monitoring pass, an integer tap (measured as unjustified — all 24 scenarios asking for `APPROXIMATED_INTEGERS` already match). See [The Remedial Action Problem](../rao/index.md) |
| **Voltage stability / continuation power flow** (P-V and Q-V curves, loadability limit) | ❌ no match for continuation/CPF/loadability in its tree (its `cpf` hits are `dcpf`) | ❌ same | ❌ same | not surveyed | not surveyed | ✅ **both curves and the loadability limit.** `src/qv.rs`: Q-V curves and the per-bus reactive margin (`gridoxide qv`, `PowerFlowModel.qv_curve`) — a fictitious condenser at the bus, its setpoint swept, the minimum interpolated through the three bracketing samples; gated against a **closed-form two-bus Q-V curve** to 1e-5 in Q. Worth knowing, and the opposite of the natural assumption: **Q-V and P-V do not agree on the weakest bus** (on `case14`, one bus in common out of five, and evaluating both at the same loading makes it worse) — they measure a system-wide collapse mode against one bus's local headroom, which is why both exist. See [Q-V Curves](../powerflow/qv.md). And `src/continuation/`: **P-V curves and the loadability limit** — tangent predictor, extended-Newton corrector over the bordered `(n+1)` system, three parametrizations (local by default — pseudo-arclength's dense bordering row measures 92x a sparse one at n = 2449), adaptive step, lower-branch tracing. `case9241pegase` traces to its nose in 10 s. Saddle-node *and* limit-induced bifurcations are distinguished; reactive-limit crossings are located exactly (bisection on arclength, matching a brute-force oracle to `1e-6` in λ against `1e-2`–`1e-1` for step-granularity switching); the weakest-bus ranking is the tangent at the nose. Gated against a **closed-form two-bus nose valid including resistance**, since no vendored reference implements continuation at all. Not yet: tap control during a trace (refused by name, `ContinuationError::TapControlUnsupported`), bidirectional PV↔PQ switching, ZIP-load derivatives (refused rather than approximated). See [Continuation Power Flow](../powerflow/continuation.md) |
...
9. **Remedial action optimization** — **Done**, and it is not a row any of the five comparison tools
   ...
   Cucumber suite, so 197 of its own scenarios are vendored verbatim and scored — **1825 of 1862**
   checkable assertions, 180/186 DC, 276/282 plus 963/977 AC, 118/123 second preventive and 288/294
   costly optimization, with **nothing** left unsupported out of what was 177 steps. Forty-odd real
   defects were found by it, every one internally consistent and externally wrong, and several only
   after a confident diagnosis had to be withdrawn. See `plans/RAO_PLAN.md` §8.3 for the list and
   [The Remedial Action Problem](../rao/index.md) for the mathematics.

   The 32 assertions that do not match are recorded disagreements rather than a backlog: each has
   been measured on the reference's own objective and in none is gridoxide worse — its
   `BestTapFinder` rounding a set-point blind to a virtual cost its own javadoc warns about, and two
   where gridoxide secures the network more cheaply than the reference asks (§8.6). The other 5 are
   one open defect, with seven mechanisms implemented, measured and refuted (§8.9).

   Genuinely absent: loop flows, relative margins, HVDC and counter-trade range actions, the
   angle/voltage monitoring pass, MARMOT multi-timestamp and CNE XML export — and an integer tap,
   which §8.21 measured as unjustified rather than assumed: all 24 scenarios whose configuration asks
   for `APPROXIMATED_INTEGERS` already match with continuous optimization and rounding, including six
   the reference built as controlled pairs against their own continuous twins.
...
10. **The analysis types with no row until this revision** — voltage stability, harmonics,
    ...
    - **Voltage stability / continuation power flow** was the cheapest by a distance, and is now
      ...
      the set answers: *how much further can this be loaded?* Q-V curves are built too (`src/qv.rs`),
      so the row is complete.
...
**Done in both domains — snapshot and batched, with voltage, power and current sensors, symmetric
and per-phase.** Within that scope gridoxide matches the most capable reference tool and leads it in
two places. What is left of the three gaps this note used to list is one part of one of them, at the
end. An earlier version claimed parity outright, which overstated it; a later one left the summary
row saying "symmetric only" for some time after the phase domain worked, which understated it in the
...
with their MPL-2.0 license files): per-unit magnitudes agree to 1.5e-9 on `transmission-case` — measured, against a standing assertion of 1e-6, since a gate is a floor and not a claim — and
...
1. **Asymmetric state estimation.** Done, and reachable. What remains is narrower than the heading
   suggests, and is a modelling difference rather than a missing capability.
   ...
   **That carry-over is now gated rather than asserted.** It was true, but it had been a claim about
   function signatures — observability, bad-data detection and the batch solver take the same
   arguments in either domain, so they *ought* to work — and the reference handling is exactly where
   a symmetric assumption would have hidden, since three per-phase rotations are three symmetries
   where `StateLayout` removes one. `tests/se_three_phase_test.rs` now checks each of them in the
   phase domain: observability finds every physical bus determined and only synthesized ones not;
   bad-data detection names the corrupted phase by a factor of five over the runner-up; and a batch
   of five scenarios matches a sequential loop bit for bit.

   Two things came out of writing those gates. **`se::case::SeCase` is the single entry point**,
   ...
   the phase domain converges at a 15σ error and diverges at 20σ, the symmetric problem converges at
   20σ and diverges at 50σ. It reports `MaxIterations` rather than a converged-looking answer, which
   is the part that matters — a diverged state carries residuals of `1e17`, and a bad-data report
   built on one is nonsense that looks like a diagnosis. A line search would raise the cliff; there
   is none, and the behaviour is gated so that adding one will announce itself.
   ...
   **Nothing is refused now, where four components used to be**, and `pgm_3ph_maps` no longer
   returns a `Result` — it could not fail, and saying otherwise in a signature is its own kind of
   wrong. Every committed fixture builds in both domains.

   The winding-pair restriction went when `transformer_seq_params` grew power-grid-model's general
   zero-sequence algorithm. `voltage_regulator` is now **ignored** rather than refused: these maps
   serve state estimation and nothing else, and a regulator pins a generator's bus and lets Q float —
   a boundary condition for a power flow, meaningless where every bus voltage is an unknown to be
   recovered. Refusing it made an estimable document inestimable for a reason that could not have
   changed the estimate, and inconsistently, since the symmetric path read the same document without
   comment. And `link` is now **modelled**: an ideal connection is phase-transparent, so its sequence
   parameters are one `branch_calc_param` repeated — which is literally what power-grid-model's own
   `link.hpp` computes, `calc_param_y_asym(y_link, 0.0, y_link, 0.0, 1.0)`. The short-circuit path
   had modelled it that way all along and carried its own copy of the loop; there is one now, shared,
   so the two cannot disagree about what a link is. Six committed state-estimation fixtures use a
   link and none of them could be estimated in the phase domain before.

   Two things came with it. A current sensor on a link is refused in the phase domain, as it already
   was symmetrically and as power-grid-model refuses it — a link's admittance is a chosen
   regularization constant, so the current through one is an artifact of that choice. And the
   Kirchhoff gate that checks the terminal functionals against the Y-bus now runs over a link-bearing
   document, with the link's admittance pinned separately against `LINK_Y` and against coupling no
   phase to any other — because Kirchhoff compares two descriptions built from the same blocks and
   would pass a link that was consistently wrong.

   And `three_winding_transformer` is **modelled**, which was the last of the four and the only one
   that was ever a real gap. It is three two-winding legs to a synthesized star node — the same star
   equivalent the symmetric path uses, `three_winding_legs` now assembling it for both so they cannot
   diverge — plus the star node itself, which is three buses like any other and injects nothing. What
   the phase domain adds is each leg's **winding configuration**, which the symmetric model never had
   to carry: T1 is `wye_n`/`wye_n` at clock 0 whatever `winding_1` is, while T2 and T3 carry their own
   winding against `winding_1` at a reversed clock, exactly as
   `three_winding_transformer.hpp::convert_to_two_winding_transformers` specifies. `PgmThreeWindingTransformer`
   grew `winding_1`/`_2`/`_3` fields to hold them; nothing had read them before.

   Gated against power-grid-model's own asymmetric answer for a three-winding transformer over ten
   tap positions — the only such reference in the vendored corpus, newly committed — and, for state
   estimation, against its published magnitudes for the fixture whose three power sensors sit on the
   transformer's three *sides*, which also exercises resolving a side to its leg.

   **What that reference cannot gate is worth stating**, because the obvious reading of it is wrong.
   Its network is a source and a transformer with no load, so the excitation is balanced and no
   zero-sequence current flows in it at all — the winding configurations, which are the whole
   difficulty, are invisible to it. Verified rather than assumed: swapping the two sides of legs T2
   and T3 leaves every number in that comparison unchanged. So the zero-sequence structure is
   asserted separately and directly, and *that* check does fail under the swap.
...
One caveat on all of the above that is about evidence rather than features. Bad-data detection is
exercised on **deliberate single gross errors** — `bad_data`'s own
`a_gross_error_is_detected_and_identified`, and, on a real fixture in the phase domain,
`bad_data_detection_finds_the_corrupted_measurement`, which perturbs one phase of one node by
fifteen sigmas and requires the culprit to rank first by a factor of five over the runner-up, with
`a_clean_phase_domain_estimate_is_not_rejected` as its other half. That is a real check and a
narrow one.

What is still missing is **realistic** noise: every benchmark and fixture here estimates from data
that is perfectly consistent apart from an injected error, so nothing measures how the detector
behaves against a plausible measurement set — several small errors at once, correlated errors, or a
gross error on a barely-redundant measurement. Nor is lead 1 *comparative*: power-grid-model has no
bad-data detection to compare against, so "better" here means "present". A noise-generating harness
does not exist.
