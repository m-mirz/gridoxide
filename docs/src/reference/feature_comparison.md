...
| **Remedial action optimization** (CRAC / CNEC, preventive + curative) | ❌ | ❌ | ❌ — this is [powsybl-open-rao](https://github.com/powsybl/powsybl-open-rao)'s job, a separate project, and it is the reference gridoxide is gated against | not surveyed (its contingency row mentions SRAP support, which is adjacent) | not surveyed | ✅ `src/rao/`: CRAC model and readers (every one of the 428 CRACs in the reference checkout imports, across 24 format versions — a recorded sweep, see `plans/RAO_PLAN.md` phase 4, not a standing gate; what CI walks is the vendored subset under `tests/data/rao`), evaluation kernel with Woodbury outage screening, LP over range actions, search tree over network actions, CASTOR perimeter decomposition, automaton simulation, MNEC soft constraints, conditional usage rules, RA usage limits, curative stop criterion, AC re-validation. second preventive with its execution conditions. Gated against the reference's **own** Cucumber suite — 197 scenarios, **1715 of 1862** checkable assertions (180/186 DC, 276/282 + 963/977 AC, 118/123 second preventive, and 178/294 on the costly-optimization corpus, which was vendored *before* that capability existed and scored 165 then). Of the 31 that differ outside that corpus, 26 are recorded disagreements measured on the reference's own objective, in none of which gridoxide is worse; the other 5 are the second-preventive corpus. Costly optimization (`MIN_COST`) for network actions. Not yet: loop flows, relative margins, HVDC range actions, a range action's cost. See [The Remedial Action Problem](../rao/index.md) |
...
9. **Remedial action optimization** — **Done**, and it is not a row any of the five comparison tools
   ...
   Cucumber suite, so 197 of its own scenarios are vendored verbatim and scored — **1715 of 1862**
   checkable assertions, 175/181 DC, 276/282 plus 963/977 AC, and 89/123 on the second-preventive
   corpus, with six steps left unsupported out of what was 177. Twenty-nine real defects were found by it, every one
   internally consistent and externally wrong, and several of them only after a confident diagnosis
   had to be withdrawn. See `plans/RAO_PLAN.md` §8.3 for the list and
   [The Remedial Action Problem](../rao/index.md) for the mathematics.

   The 26 assertions that do not match are recorded disagreements rather than a backlog: each has
   been measured on the reference's own objective and in none is gridoxide worse — mostly its
   `BestTapFinder` rounding a set-point blind to a virtual cost its own javadoc warns about
   (§8.6). Genuinely absent: second-preventive optimization, loop flows, relative margins, HVDC range
   actions and costly optimization.
