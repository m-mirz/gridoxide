...
1. **Asymmetric state estimation.** Done, and reachable. What remains is narrower than the heading
   ...
   **One component is refused now, where four used to be.** `pgm_3ph_maps` reports a typed error
   rather than dropping anything silently, and the list it reports has shrunk to
   `three_winding_transformer`, which genuinely needs asymmetric star parameters.
