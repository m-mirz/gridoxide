...
3. **Distributed slack** — 4 of 5 tools have it (lightsim2grid, powsybl, VeraGrid, pandapower); real transmission grids split slack across several generators, and a single slack absorbing an unscheduled few hundred megawatts distorts every branch flow around it. **Done** — `solver::newton_raphson_distributing_slack`, an outer loop in the same shape as the Q-limit one: solve, compare what the slack actually produced against what its own `p_spec` schedules it for, move the difference onto the participating generators by normalized weight, solve again.
