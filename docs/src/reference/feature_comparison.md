...
8. **DC-OPF** (`opf::dc`, `opf-highs` feature) — least-cost dispatch as a convex QP, with locational marginal prices and binding-limit shadow prices, behind a solver-independent boundary backed by HiGHS. Validated against analytic cases, KKT certificates and pglib-opf's published DC baseline (all five cases inside 0.03%). See [Optimal Power Flow](../opf/index.md).

   Still absent: **AC-OPF** (nonconvex, and needs injection Hessians the sensitivity module stops short of), unit commitment, and security-constrained OPF.
