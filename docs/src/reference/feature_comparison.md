...
3. **Distributed slack** — 4 of 5 tools have it (lightsim2grid, powsybl, VeraGrid, pandapower); real transmission grids split slack across several generators, and a single slack absorbing an unscheduled few hundred megawatts distorts every branch flow around it. **Done** — `outerloop::DistributedSlack`, an outer loop in the same shape as the Q-limit one: solve, compare what the slack actually produced against what its own `p_spec` schedules it for, move the difference onto the participating generators by normalized weight, solve again.
   ...
   Still absent: powsybl's area-interchange control. Area interchange is the same mechanism generalized from one balance target to several rather than a different one — powsybl's `AcAreaInterchangeControlOuterLoop` literally constructs a `DistributedSlackOuterLoop` as its no-area fallback and shares its `ActivePowerDistribution` engine, so what would be needed here is an area assignment, a scheduled interchange per area, and a per-area mismatch from tie-line flows in place of the slack deviation. See [Distributed Slack](../powerflow/distributed_slack.md).
